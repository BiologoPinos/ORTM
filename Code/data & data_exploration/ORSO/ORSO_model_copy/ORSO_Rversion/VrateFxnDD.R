...
somatrix2st <- function(vrates){
  ...
  fs = vrates[5:6]
  ms = vrates[7:8]
  SD = 2.5; # stage duration for sub-adult
...
lams = numeric()
btar = numeric(); 
L = c(0, 1.02, 0, .50, .68, .84, .65, .83) 
H = c(0, 1.0, 0, .90, .94, .96, .92, .92) 
# R-max [Kodiak]
vrates[1,] = H 
rslt = somatrix2st(vrates[1,])   
lams[1] = rslt$lam; Gvals[1,] = rslt$G;  
# Stable/Declining pop [Amchitka]
vrates[2,] = L 
rslt = somatrix2st(vrates[2,]) 
lams[2] = rslt$lam; Gvals[2,] = rslt$G;  
##
rpar = cbind(runif(reps,0,18),runif(reps,0,18))
for (i in 3:reps){
  rpar = runif(1,1,49)
  btar = rbeta(8,50-rpar,rpar)
  vrates[i,] = L*btar + H*(1-btar)
  rslt = somatrix2st(vrates[i,]) 
  lams[i] = rslt$lam; Gvals[i,] = rslt$G;  
}
lams2 = lams^2
Demdat = data.frame(Lam=round(lams,2),br1 = vrates[,1],br2 = vrates[,2],
