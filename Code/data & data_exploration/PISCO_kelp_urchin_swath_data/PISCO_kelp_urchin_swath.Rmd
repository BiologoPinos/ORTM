...
kelp_adult <- swath %>%
  ...
  summarise(plants   = sum(plants),
            stipes   = sum(stipes),
            biomass  = stipes * 5.18,  # 5.18kg per stipe (add a range of variation???)
            .groups = "drop")
...
#Step 2: Mean successful recruitment across space and time
...
# Step 1: join tables
KU_OR <- kelp_OR %>% 
  full_join(urchin_OR, by = c("year", "site", "zone", "transect"))

# Step 2: compare kelp v urchins biomasses

  # plot purples alone
  ggplot(KU_OR) +
    geom_point(aes(y = biomass, x = purp_biomass, colour = as.factor(year)) )
  
  # plot reds alone
  ggplot(KU_OR) +
    geom_point(aes(y = biomass, x = red_biomass, colour = as.factor(year)) )
  
  # plot both urchins (reshape table first)
  KU_OR1 <- KU_OR %>% select(year:transect, biomass, red_biomass, purp_biomass) %>%
    pivot_longer(red_biomass:purp_biomass, names_to = "urchin_type", values_to = "urchin_biomass") %>% 
    mutate(across(urchin_type,~str_extract(., ".*(?=(_))"))) %>% 
    mutate(urchin_type = factor(urchin_type, levels = c("purp","red"), labels = c("purple","red")))
  
  ggplot(KU_OR1) +
    geom_point(aes(y = biomass, x = urchin_biomass, colour = urchin_type)) +
    theme_bw() +
    labs(x = "Urchin biomass (kg.60m2)", y = "Bull kelp biomass (kg.60m2)", color = "Urchin Type\n")
