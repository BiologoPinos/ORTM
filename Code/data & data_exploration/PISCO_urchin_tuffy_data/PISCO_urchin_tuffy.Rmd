...
##AIM OF THE SCRIPT
Obtain the year to tear variability of urchin recruitment (SD) in Oregon
...
```{r}
# Load PISCO data
pisco <- read_csv("PISCO_all_years.csv")

# Filter for urchin taxa and remove unwanted sites
urchins_raw <- pisco %>%
  filter(count_classcode %in% c("66", "67", "70"),
         !site_code %in% c("CMEN00", "CMES00", "IFBRXX", "KHLX00",
                           "PSGX00", "SHCX00", "TRHX00", "IPTAXX"))

# Aggregate counts per transect, then per year
urchins <- urchins_raw %>%
  group_by(year, site_code, zone, replicate) %>%
  summarise(count = sum(count), .groups = "drop") %>%
  mutate(log_count = log(count + 1))  # Log-transform counts
...
Objective:
1. estimate the normalized year to year variation (SD) across sites of incoming recruits
Note: sea urchin recruitment follows a non-normal distribution

Aim:
I'd be building a hierarchical structure where my incoming settlers data informs
a non-normal distributed annual means, and those annual means inform a normally 
distributed year to year variation (SD).

Model(s) method(s):
1. NIMBLE
2. BRMS

## 1) NIMBLE VERSION OF THE MODEL
```{r}
# Load libraries
library(nimble)
...
model_code <- nimbleCode({
  ...
  for (s in 1:n_site) {
    b_site[s] ~ dnorm(0, sd = sigma_site)
  } #s
  
  # Random intercept for each site×year combo:
  for (j in 1:n_sy) {
    b_sy[j] ~ dnorm(0, sd = sigma_sy)
  } #j
  
  # “Normalized” (original‐scale) version of sigma_sy:
  sigma_sy_n <- exp(sigma_sy) 
  
  # Likelihood: log(count+1) ~ Normal:
  for (i in 1:N) {
    mu[i]     <- beta0 + b_site[site[i]] + b_sy[sy_index[i]] #process model
    log_y[i] ~ dnorm(mu[i], sd = sigma_obs) #observation model
  } #i
...
# Prepare input data
urchins <- urchins %>%
  mutate(site = as.integer(factor(site_code)),
         site_year = paste(site_code, year, sep = "_"),
         sy_index = as.integer(factor(site_year)))

log_y    <- urchins$log_count
site     <- urchins$site
sy_index <- urchins$sy_index
N        <- nrow(urchins)
n_site   <- length(unique(site))
n_sy     <- length(unique(sy_index))

nimble.data <- list(log_y = log_y, 
                    site = site, 
                    sy_index = sy_index)

nimble.constants <- list(N = N, 
                         n_site = n_site, 
                         n_sy = n_sy)
...
# Parameters
parameters <- c("beta0", "sigma_site", "sigma_sy", "sigma_sy_n", "sigma_obs")

# Run MCMC
mcmc.output <- nimbleMCMC(code      = model_code, 
                          data      = nimble.data,
...
# Summarize posterior
as.mcmc(mcmc.output)
summary(mcmc.output$samples)

# Extract (normalized) yearly variation across sites
mean(sigma_sy_n) #Number we want

# Posterior hist
hist(sigma_sy_n, main = "Posterior of Normalized Year-to-Year SD",
     xlab = "Normalized SD")
hist(sigma_sy, main = "Posterior of Non-Normal Year-to-Year SD",
     xlab = "Non-Normal SD")
...
MCMCtrace(object = mcmc.output$samples,
          ...
          ind = TRUE,
          # params = "sigma_sy_n")
          params = c("beta0", "sigma_site", "sigma_sy", "sigma_sy_n", "sigma_obs")) #Posterior predictive check

gelman.diag(mcmc.output$samples, multivariate = FALSE)
...
# Fit zero-inflated Poisson model with nested random effects
brms_model <- brm(
  formula = count ~ site_code + (1 | site_code / year), #Nested (site‐specific) Year Variance
  #formula = count ~ site_code + (1 | site_code) + (1 | year), #Global (pooled) Year Variance
...
# Posterior draws for yearly variation across sites
post <- as_draws_df(brms_model, transform = TRUE) #FALSE for log scale
mean(post[["sd_site_code:year__Intercept"]]) #number we want

```


## OLD VERSIONS WITH THE WRONG APPROACH (ignore)
<!-- ## WORKING VERSION OF THE MODEL (WRONG APPORACH) -->
<!-- ```{r} -->
<!-- # Load nimble -->
<!-- library(nimble) -->
<!-- source("attach.nimble.R") -->
...
<!-- hist(sd_eps, main = "Title", -->
<!--      xlab = "Normalized SD") -->
<!-- mean(sd_eps) -->
<!-- quantile(sd_eps, probs = c(0.025, 0.5, 0.975)) -->
...
<!-- ``` -->
<!-- ## POSTERIOR PREDICTIVE CHECK (WRONG APPORACH) -->
<!-- ```{r} -->
...
<!-- ``` -->
<!-- ## OLDER VERSION OF THE MODEL (WRONG APPORACH) -->
<!-- ```{r} -->
<!-- # Nimble model - Fun begins -->
<!-- model_code <- nimbleCode({ -->
