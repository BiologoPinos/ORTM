...
%% Plot SIMS persistence (Kelp 🌿) over time (for multiple reps, single scenario)
    ...
    % Plot
        ...
        % Over seasons? (note: baseline always at the end)
            % plot((1:T2+1), sum(kt2(2, :, :)>0, 3)./RR, 'k', 'LineWidth', 4) % baseline
            % plot((1:T2+1), sum(kt2(2, :, :)>0, 3)./RR, 'b', 'LineWidth', 4) % management (note: choose color)

        % Over years
            plot(((0:T2)/4), sum(kt2(2, :, :)>0, 3)./RR, 'c--', 'LineWidth',4) % baseline
            % plot(((0:T2)/4), sum(kt2(2, :, :)>0, 3)./RR, 'b', 'LineWidth',4) % management (note: choose color)

        % When does management starts?
            % xline((dist.yrs(1)/4),'--','Mng. Period','Color','k','LabelHorizontalAlignment','left','LabelVerticalAlignment','bottom','FontSize',26) % start of management (years)
            % xline((dist.yrs(1)/4)+2,'--','Color','k','LabelHorizontalAlignment','left','LabelVerticalAlignment','bottom','FontSize',26) % start of management (years)
            % xline(dist.yrs(1),'--','Start Mng','Color','k','LabelHorizontalAlignment','left','LabelVerticalAlignment','bottom','FontSize',22) % start of management (seasons)
            % xline(buffer/4,'--','Sea Otter Reintroduction','Color','k','LabelHorizontalAlignment','left','LabelVerticalAlignment','bottom','FontSize',26) % Sea otter reintroduction (years)
