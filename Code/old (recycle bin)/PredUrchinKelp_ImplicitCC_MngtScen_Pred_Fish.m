% PredUrchinKelp_ImplicitCC_MngtScen_Pred_Fish.m
...
% load in parameter values
    ...
    kelp = ParaKelp_Implicit(tmax);
        % NOTE: make sure kelp spp matches lines 109-115 "kelp initial conditions"

    % urchin parameters
    urchin = ParaUrchin_Implicit(tmax);

    % predation parameters source 
    % either: simulated predator or dataset (forced) predator
        % 1) simulated predator ['Blue rockfish'&'Sheephead']
        pred = ParaPred_Implicit(tmax); % NOTE: everything that is "PBE" or related is for this scenario 
        % % % 2) dataset (forced) predator [sea otter]
        % %     % choose dataset:
...
% initial conditions:
    % KELP 🌿 (NOTE: choose between giant and bull kelp)
        ...
        % % % Bull kelp:
        % % kt0 = [3.17*10^5,3.17*10^5,3.17*10^6]; %
        % Giant kelp:
...
% pre-assign variables:
    ...
    %% ASK JESS WHAT kts uts kelp_avg and PBE stands for?
    %
